...
import {
  ...
  remove,
  post,
} from "@sellgauge/common/services/axios";
...
export default function rest<T>(modelName: string, listMethod = "get") {
  return {
    ...
    async list(
      ...
      const runner = { get: get, post: post }[listMethod];

      const response = await runner<(T & IMongoDocument)[]>(`${modelName}`, {
        headers,
